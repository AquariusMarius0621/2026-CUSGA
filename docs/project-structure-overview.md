...
继续这轮后，`MainMenuController` 在“页面已成型”阶段也改成了显式引用优先：它不再按对象名回填主菜单子节点，只在缺引用时输出告警；对象名现在主要只服务于首次生成默认骨架。
...
当前关卡现在还会在“场上还没有任何已建塔”时，于地图上常驻显示首塔起手区标记；第一座塔放下后会自动隐藏。
编辑状态下，`GameController` 还会通过 Scene Gizmos 直接把这块起手区画出来，所以不进 Play 也能在 Scene 视图里看见。
...
可以把它理解成“导演”。
当前它也已经开始第一阶段去掉“按对象名查找场景对象”的迁移：核心场景依赖现已支持 Inspector 直拖优先，`SampleScene` 里关键运行时引用也已经接上。
几乎所有主流程最后都会收口到这里。
当前它也已经开始从“按对象名查找”迁到“显式场景引用优先”的模式，`SampleScene` 里的核心运行时对象和一批主 HUD 引用都已经接上。
继续推进后，`TowerDefenseGame` 和 `WaveSpawner` 的主场景依赖已经不再靠名字查找拼主链；当前剩余的名字兜底主要集中在 HUD 表现层。
继续这一轮后，HUD 表现层也改成了“显式绑定引用 + 缺项告警”的模式，所以当前主玩法场景的核心运行链路已经基本不再主动依赖对象名查找。
同时，`TowerDefenseGame` 也不再给 HUD presenter 传一整套对象名配置，主链代码表达会更直接。
继续这轮后，HUD presenter 自己那层旧名字字段也已经清掉了，所以这条主链现在从代码结构上也更接近真正的显式装配。
...
当前它也已经支持“外部显式注入 HUD 引用，再对缺失项 fallback 名字查找”的过渡模式。

如果你想改：
- 顶部资源区样式
- 右侧发电机/炮塔卡片排版
- 拖拽提示文案和位置
- 常驻状态条对象已从 HUD 层级中物理删除
就优先看这里。
...
这一轮继续收尾后，它也不再根据对象名推断塔类型；部署卡身份现在必须由 Inspector 显式配置。
...
### `Enemy.cs`
当前它的血条链路也已经从“按子节点名查找”迁到“EnemyPrototype 显式序列化 Root / Fill / Background 引用”，后续改血条子节点名时更安全。
敌人本体。
负责：
- 沿路径移动
- 受伤/死亡
...
当前它仍然存在，但现在更适合作为过渡期 fallback，而不再是后续长期架构的首选依赖方式。
